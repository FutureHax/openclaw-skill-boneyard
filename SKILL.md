...
Post character death announcements to the **#boneyard** Discord channel when a player's character falls during a game session.
...
After gathering all required info, **confirm the details** with the user, then run:
...
- The boneyard channel is hardcoded in the tool — no need to specify a channel ID
